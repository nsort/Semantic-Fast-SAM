...
# load the saved model for the classifier
classifier = SimpleClassifier(input_dim=768, num_classes=4)
...
for images_batch, anns_batch, image_ids_batch in tqdm(train_loader):
    ...
    for idx in range(batch_size):
        ...
        for fs_mask in fs_masks:
            ...
            predicted_labels.append(torch.argmax(classifier_output).item() + 1)  # Convert back to 1-based index
        ...
        imshow_det_bboxes(
                image,
                bboxes=None,
                labels= predicted_labels,#np.arange(len(bitmasks)),
